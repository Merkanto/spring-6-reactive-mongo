...
import merkanto.spring6reactivemongo.services.BeerService;
...
public class BeerHandler {
    ...
    public Mono<ServerResponse> patchBeerById(ServerRequest request) {
        return request.bodyToMono(BeerDTO.class)
                ...
                .flatMap(beerDTO -> beerService
                        .patchBeer(request.pathVariable(BEER_ID), beerDTO))
                .switchIfEmpty(Mono.error(new ResponseStatusException(HttpStatus.NOT_FOUND)))
    ...
    public Mono<ServerResponse> updateBeerById(ServerRequest request) {
        return request.bodyToMono(BeerDTO.class)
                .doOnNext(this::validate)
                .flatMap(beerDTO -> beerService
                        .updateBeer(request.pathVariable(BEER_ID), beerDTO))
                .switchIfEmpty(Mono.error(new ResponseStatusException(HttpStatus.NOT_FOUND)))
                .flatMap(savedDto -> ServerResponse.noContent().build());
    }
    ...
    public Mono<ServerResponse> getBeerById(ServerRequest request){
        return ServerResponse
                .ok()
                .body(beerService.getById(request.pathVariable(BEER_ID))
    ...
    public Mono<ServerResponse> listBeers(ServerRequest request) {
        ...
        if (request.queryParam(BEER_STYLE).isPresent()) {
            flux = beerService.findByBeerStyle(request.queryParam(BEER_STYLE).get());
        } else {
